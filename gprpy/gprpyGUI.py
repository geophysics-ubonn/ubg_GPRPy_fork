...
class DeWowGui(object):
    """Provide a popup window for selecting DeWow  filter properties
    """
...
class GPRPyApp:
    ...
    def __init__(self, master):
        ...
        a.tick_params(
            direction='out', length=6*self.widfac, width=self.highfac
        )
        ...
        self.balloon.bind(
            AspButton, "Set the aspect ratio between x- and y-axis."
        )
        ...
        LoadButton.grid(
            row=0, column=rightcol, sticky='nsew', columnspan=colsp, rowspan=2
        )
        ...
        AdjProfileButton.grid(
            row=2, column=rightcol, sticky='nsew', columnspan=colsp
        )
        ...
        SetZeroTimeButton.grid(
            row=3, column=rightcol, sticky='nsew', columnspan=colsp
        )
        ...
        TrAlignButton.grid(
            row=4, column=rightcol, sticky='nsew', columnspan=colsp
        )
        self.balloon.bind(
            TrAlignButton,
        ...
        truncYButton.grid(
            row=5, column=rightcol, sticky='nsew', columnspan=colsp
        )
        self.balloon.bind(
            truncYButton,
        ...
        DewowButton.grid(
            row=7, column=rightcol, sticky='nsew', columnspan=colsp
        )
        self.balloon.bind(
            DewowButton,
        ...
        remMeanTraceButton.grid(
            row=8, column=rightcol, sticky='nsew', columnspan=colsp
        )
        self.balloon.bind(
            remMeanTraceButton,
        ...
        SmoothButton.grid(
            row=9, column=rightcol, sticky='nsew', columnspan=colsp)
        self.balloon.bind(
            SmoothButton,
        ...
        profSmButton.grid(
            row=10, column=rightcol, sticky='nsew', columnspan=colsp
        )
        self.balloon.bind(
            profSmButton,
        ...
        hypButton.grid(
            row=12, column=rightcol, sticky='nsew', columnspan=colsp
        )
        ...
        antennaSepButton.grid(
            row=14, column=rightcol, sticky='nsew', columnspan=colsp)
        self.balloon.bind(
            antennaSepButton,
        ...
        migButton.grid(
            row=15, column=rightcol, sticky='nsew', columnspan=colsp)
        self.balloon.bind(
            migButton,
        ...
        topoCorrectButton.grid(
            row=16, column=rightcol, sticky='nsew', columnspan=colsp)
    ...
    def profileSmooth(self, proj):
        ...
        if ntraces is not None:
            noversample = sd.askinteger(
                "Input",
                "Make how many copies of each trace?\nRecommended: Same " +
                "as number of traces to be smoothed."
            )
    ...
    def topoCorrect(self, proj):
        if proj.velocity is None:
            mesbox.showinfo(
                "Topo Correct Error", "You have to set the velocity first"
            )
            return
    ...
    def exportVTK(self, proj):
        ...
        if outfile is not '':
            ...
            if proj.threeD is None:
                gpyes = mesbox.askyesno(
                    "Question",
                    "Do you have topography data for this profile?"
                )
                if gpyes:
                    filename = fd.askopenfilename()
                    self.getDelimiter()
    ...
    def plotProfileData(self, proj, fig, a, canvas):
        ...
        if proj.velocity is None:
            a.imshow(
                ...
                cmap=self.color.get(),
                extent=[
                    min(proj.profilePos)-dx/2.0,
                    max(proj.profilePos)+dx/2.0,
                    max(proj.twtt)+dt/2.0,
                    min(proj.twtt)-dt/2.0
                ],
                aspect="auto",
                vmin=-stdcont/self.contrast.get(),
                vmax=stdcont/self.contrast.get()
            )
        ...
        if self.picking:
            ...
            a.plot(
                ...
                '-x',
                color='black',
                linewidth=2*self.highfac
            )

        # Allow for cursor coordinates being displayed
        def moved(event):
            if event.xdata is not None and event.ydata is not None:
                canvas.get_tk_widget().itemconfigure(
                    tag,
                    text="(x = %5.5g, y = %5.5g)" % (event.xdata, event.ydata)
                )
    ...
    def getDelimiter(self):
        ...
        text = tk.Label(
            commaQuery,
            text="Is this a comma- or tab-separated file?",
            fg='red'
        )
